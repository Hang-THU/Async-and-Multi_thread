...
public class Database
{
    ...
    public async Task UpdateInventoryAsync(string title, int quantity)
    {
        ...
        lock (_lock)
        {
            ...
            {
                Console.WriteLine($"库存不足：{title}，请求 {quantity} 本，剩余 {book.Inventory} 本");
                return;
            }
            book.Inventory -= quantity;
            Console.WriteLine($"购书成功：{title}，购买 {quantity} 本，剩余 {book.Inventory} 本");
        }
    }
}
...
public class BookStore
{
    ...
    public async Task SimulateMultipleUsers()
    {
        ...
        {
            CheckoutAsync("C#入门", 2),   // 用户1
            CheckoutAsync("C#入门", 3),   // 用户2
            CheckoutAsync("异步编程", 1), // 用户3
            CheckoutAsync("异步编程", 2), // 用户4
            CheckoutAsync("异步编程", 3)  // 用户5
        };
